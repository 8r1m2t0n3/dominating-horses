...
void main() {
	...
	while (1) {
		...
		scanf("%d", &num);
		if (num <= 3) printf("Field configuration is too small.\n");
		if (num > 16000) printf("Field configuration is too large.\n");
		else break;
	}
	...
	while (1) {
		...
		case 2:
			...
			else printf("There is no horse in the cell.\n");
			break;
		case 3:
			printf("\nField representing positions of the horses (each '2' represents horse piece):\n");
			print_field(field);
			printf("\nField representing cells that are threatened (ach number represents the number of times a cell is threatened.):\n");
